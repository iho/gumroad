...
type ProductLoaderConfig struct {
	...
	Fetch func(keys []int32) ([]*pg.Product, []error)
...
func (l *ProductLoader) Load(key int32) (*pg.Product, error) {
...
func (l *ProductLoader) LoadThunk(key int32) func() (*pg.Product, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*pg.Product, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *ProductLoader) LoadAll(keys []int32) ([]*pg.Product, []error) {
	results := make([]func() (*pg.Product, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	products := make([]*pg.Product, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *ProductLoader) LoadAllThunk(keys []int32) func() ([]*pg.Product, []error) {
	results := make([]func() (*pg.Product, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *ProductLoader) Prime(key int32, value *pg.Product) bool {
	l.mu.Lock()
	var found bool
...
func (l *ProductLoader) Clear(key int32) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}

func (l *ProductLoader) unsafeSet(key int32, value *pg.Product) {
	if l.cache == nil {
		l.cache = map[int32]*pg.Product{}
	}
	l.cache[key] = value
}
...
func (b *productLoaderBatch) keyIndex(l *ProductLoader, key int32) int {
